...
export function useWorkflow() {
  ...
  // ─────────────────────────────────────────────────────────────
  // 🔥 CLEAN EDGES — removes stale edges with old handle IDs
  // ─────────────────────────────────────────────────────────────
  const cleanEdges = (rawEdges: Edge[]): Edge[] =>
    rawEdges.filter((e) => {
      // Remove old "output-0" sourceHandle — TextNode now uses "output"
      if (e.sourceHandle === "output-0") {
        console.warn(`🧹 Removing stale edge [output-0]: ${e.id}`);
        return false;
      }
      return true;
    });
  ...
  const saveWorkflow = async (name: string) => {
    ...
    const safeEdges = cleanEdges(edges); // 🔥 clean before saving to DB
    ...
    if (workflowId) {
      ...
      console.log("🔗 Edges     :", safeEdges.length, safeEdges);
    ...
    const payload = { name: uniqueName, nodes: safeNodes, edges: safeEdges };
    ...
    if (data.data?.id) {
      ...
      setWorkflow(nodes, safeEdges, data.data.id, uniqueName);
  ...
  // ✅ LOAD — cleans stale edges on load
  const loadWorkflow = (workflow: Workflow) => {
    console.group(`📂 LOAD workflow: "${workflow.name}" [${workflow.id}]`);

    // 🔥 Remove stale edges with old handle IDs from DB
    const safeEdges = cleanEdges(workflow.edges);

    console.log("🟦 Nodes:", workflow.nodes.length);
    console.log("🔗 Edges (after clean):", safeEdges.length);
    console.groupEnd();

    setWorkflow(workflow.nodes, safeEdges, workflow.id, workflow.name);
  ...
  // 🔥 RESTORE — cleans stale edges on restore
  const restoreLastWorkflow = async () => {
    ...
    if (match) {
      // 🔥 Clean stale edges before restoring
      const safeEdges = cleanEdges(match.edges);
      console.log(`✅ RESTORE — found "${match.name}", loading...`);
      setWorkflow(match.nodes, safeEdges, match.id, match.name);
    } else {
